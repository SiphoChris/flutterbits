...
void main() {
  ...
  test('a matching state layer overrides the base (last-wins)', () {
    ...
    expect(
      style.resolve(<WidgetState>{WidgetState.hovered}).background,
      _b,
    );
  });
  ...
  test('container layer keys off containerWidth, not viewportWidth', () {
    ...
    expect(
      style.resolve(const <WidgetState>{}, containerWidth: 800).background,
      _b,
    );
  });
  ...
  test('nested md:hover resolves jointly', () {
    ...
    expect(
      style
          .resolve(<WidgetState>{WidgetState.hovered}, viewportWidth: 800)
          .background,
      _b,
    );
    ...
    expect(
      style
          .resolve(<WidgetState>{WidgetState.hovered}, viewportWidth: 500)
          .background,
      _a,
    );
  });
