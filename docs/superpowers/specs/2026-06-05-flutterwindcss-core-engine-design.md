...
- **ClipRRect geometry (Finding #3):** the content clip reuses the decoration's `BorderRadiusDirectional`, deflated by `borderWidth`, so clipped content never bleeds across the stroke. *(Status: the deflation **lands with the border utilities in module 5** — it is coupled to the border width that module 5 introduces, and was untestable in module 3 where no `.border`/`.rounded`/`.clip` setters ship. Until then the chain emits the content clip **without** deflation. Tracked, not silently dropped.)*
...
| 3 | **Resolver core** ✅ landed | `FwStyle`, `ResolvedStyle`, `FwStyled`, render chain, the **nested** layering engine + precedence (incl. disabled suppression, joint `md:hover:`), **two-width** resolve (viewport vs container kept distinct), conditional `MediaQuery`/`LayoutBuilder`/interaction-sourcing (`MouseRegion`+non-traversable-`Focus`+`Listener`) insertion. The `.tw` entry. Ships the **padding + bg** base slice + the full variant surface; modules 4–9 add the rest. Tests for last-wins, precedence, nested resolution, render order, sizing reconciliation, focus-traversal hygiene, RTL + semantics transparency. **Deferred to M5** (coupled to border width): content-clip radius **deflation** (Finding #3). **Deferred (perf-only)**: opacity fold (Finding #11) — an always-correct `Opacity` is emitted meanwhile. |
| 4 | **Spacing + sizing** | margin, w/h/min/max, fractional (+ alignment), aspect setters (the sizing reconciliation + the render-chain wrappers already exist from module 3; this adds the typed `.tw` setters). (`padding` already shipped as module 3's slice.) Unit + golden. (`gap` lands with the flex widgets in module 8.) |
| 5 | **Color + border + radius + gradient** | gradient, border (directional), radius (directional + named + full), using semantic tokens. (`bg` already shipped as module 3's slice.) **Also lands the content-clip radius deflation by border width (Finding #3), deferred from module 3.** Unit + golden (light/dark). |
