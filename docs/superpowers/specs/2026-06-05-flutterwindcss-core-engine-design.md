...
- **`FwGrid`** — the AGENTS.md §11 grid helper. **v1 grammar (pinned):** a single set of **column tracks** mixing `fr` (flex) and fixed-px tracks via the sealed **`FwGridTrack`** type — `[FwFr(1), FwFr(2)]` or `[FwPx(200), FwFr()]` (corrected — module 8: the spec's bare `Fr`/`Px` violated the AGENTS.md §4 `Fw`-prefix rule; a *sealed* base lets the cell builder `switch` exhaustively) — with a directional **column gap** and equal-structure wrapping rows (a partial final row is padded with empty cells), all implementable with `Flex`/`Expanded` (`FwFr` → `Expanded(flex:)`, `FwPx` → `SizedBox(width:)`). `columnGap`/`rowGap` are utility units; `FwPx` size is logical px. `FwGrid`'s constructor is **not `const`** (its non-empty-`columns` guard inspects the list). **Cell/row spanning, auto-placement, and `subgrid` are Non-Goals** (AGENTS.md §11) and require a custom `RenderObject` we are *not* shipping in v1. "FwGrid ships complete" means complete *for this grammar*, stated so it's falsifiable.

**Responsive layering on layout widgets (corrected — module 8, deferred):** v1 layout widgets take **concrete** values (`gap`, alignment, tracks). **Box-level** responsiveness still works by chaining `.tw` on the widget (`FwRow(...).tw.md((s) => s.p(4))`). Responsive *layout-property* layering (a `gap` that changes at `md:`) would require each layout widget to host the `FwStyle` layer engine + `MediaQuery`/`LayoutBuilder`; it is **deferred** (out of v1 scope) and documented as a limitation rather than half-built — the upgrade path is to host an `FwStyle` layer inside the layout widget in a later module. All widgets are directional; all golden-tested LTR + RTL.
...
The barrel re-exports exactly the supported surface (AGENTS.md §3.6): `context.fw`, the `.tw` extension + `FwStyled`, `FwStyle` (for advanced composition) and its public value types (`FwBorderSpec`, `FwLayer`/`FwCondition`), the layout widgets (`FwRow`, `FwColumn`, `FwWrap`, `FwStack`, `FwPositioned`, `FwGrid`) and the grid track grammar (`FwGridTrack`/`FwFr`/`FwPx` — added module 8), `FwTokens/FwColors/FwRadii/FwShadows/FwTypography`, `FwPalette`, `FwTheme`, `FwThemeExtension`, `FwAnimatedTheme`, the enums/scales (`FwBreakpoint`, `FwState`, `FwBlur`, `FwFontSize`, …), and `fwSpace`. (There is no `FwShadow` enum — corrected, module 7; the shadow scale is the exported `FwShadows` class, and `shadow()` takes a resolved `List<BoxShadow>`.) Nothing under `lib/src/` is importable by consumers. Adding to the surface is always allowed; **renaming/removing requires a deprecation cycle** (every copied component in the wild pins these names).
...
| 8 | **Layout widgets** ✅ landed | `FwRow`/`FwColumn`/`FwWrap`/`FwStack`/`FwPositioned`/`FwGrid` (flex/gap/align, positioning/inset/z, grid tracks) as dedicated multi-child widgets (§6.6). As-built: `gap`/spacing via **native `Flex.spacing`** (not interleaved `SizedBox`); grid grammar is the sealed **`FwGridTrack`** (`FwFr`/`FwPx`), not bare `Fr`/`Px`; `FwStack` z-order is a **stable** sort honoring `fwZIndices`; responsive *layout-property* layering **deferred** + documented (box-level responsiveness via `.tw` works). The **`.containerSm…` query family was already delivered in module 3** (the row-8 bundling was redundant — `FwContainerCondition` + the `LayoutBuilder`/`containerWidth` path + the setters all shipped then), so it is not re-built here. Unit (`fw_flex/wrap/stack/grid_test`) + golden (`layout_slice`, LTR + RTL × light/dark). |
