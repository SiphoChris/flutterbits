...
| `FwLayer`, `FwCondition` | `lib/src/style/fw_layer.dart` | `FwLayer = (FwCondition, FwStyle)`; `FwCondition` = `state(WidgetState)` \| `viewport(FwBreakpoint)` \| `container(FwBreakpoint)` \| `group/peer(FwGroupCondition)` (4th member added — module 14; relation + state + name, matched against the `FwGroup` scope channels). Layers nest (a layer's style may itself carry layers). |
