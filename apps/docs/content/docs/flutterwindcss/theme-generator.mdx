...
description: Paste a tweakcn/shadcn theme and copy a working Flutter theme.dart for flutterwindcss — colors, radius, shadows, and fonts, with nothing dropped.
---
...
- **Colors — all 32.** The 19 core roles plus `chart-1…5` and the 8 `sidebar-*` tokens, mapped
  ...
  generator **won't produce a `theme.dart`** until all 32 are present (see below).
...
<Callout type="info" title="Fonts are named, not bundled">
  ...
  a clearly-commented [`google_fonts`](https://pub.dev/packages/google_fonts) wiring stub. Until you
