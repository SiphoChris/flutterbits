...
- **Review and audit**.  yourself after every change made to see ensure proper direction and position of the project.
- **Fix a wrong spec with judgment — don't silently diverge.** Specs and plans are decisions, not scripture: when implementation proves one *wrong, impossible, or weaker than achievable* (e.g. a named Flutter widget can't satisfy a stated guarantee, or a signature is a latent bug), **correct the spec** to the better design and record *why* (a short "corrected — module N" note inline, plus the PR description). Do **not** quietly build something different from what the doc says. Two guardrails: (1) this licenses fixing **mistaken** specs, never weakening the **§3 hard rules** to ship faster (those still fail review if broken — see below); (2) if the right call is genuinely ambiguous or a user-facing trade-off, surface it instead of deciding unilaterally.
