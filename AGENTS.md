...
- Radius is derived from one base value (shadcn `--radius`). The engine's `FwRadii.fromBase` uses `×0.6/0.8/1.0/1.4` factors (these match shadcn's *additive* derivation only at the 10px default); the generator emits **explicit additive** values (`sm = r−4px, md = r−2px, lg = r, xl = r+4px`, clamped ≥ 0) per §7. Components use `t.radii.md` etc., never a literal radius.
...
- The semantic set is not the whole token system. A `FwTokens` bundle carries `colors` (the 32 above), `radii`, `shadows` (`FwShadows`), and `typography` (`sans`/`serif`/`mono` family names **plus `tracking`** (em letter-spacing) — flutterwindcss bundles no fonts, so the generator emits a `google_fonts` wiring stub rather than a silent fallback, per §7). **Theme-independent** scales live on their own types: the raw Tailwind v4 palette (`FwPalette`, baked — used to *build* themes, never referenced directly by components), the Tailwind named radius/typography/shadow/blur scales, and scalar scales for opacity, border-width, z-index, and breakpoints. The interaction-state and breakpoint enums (`FwState`, `FwBreakpoint`) are a **frozen API contract** — exhaustive `switch`es depend on them, so adding a value is a breaking change, not additive.
...
- Input: a pasted tweakcn/shadcn theme (`:root` + `.dark` blocks). Output: a **`theme.dart`** the dev copies (shadcn-style — the web UI shows the source with a Copy button, no file download). `theme.json` (the `FwTokens` schema) remains the **internal source of truth** the Dart is emitted from and drives the preview, but is **not** surfaced to the dev (UX decision 2026-06-09 — devs don't need the JSON).
...
- Color conversion: OKLCH → OKLab → linear sRGB → gamma-encode → sRGB.
  ...
  - **Optional = perceptual.** A `mode: 'perceptual'` on the `generateTheme`/`parseCssColor` library API **gamut-maps** (chroma reduction, hue-preserving) instead of clipping, for extreme out-of-gamut colors or a display-P3 target, where naive clipping could shift hue. This is also where the wider-gamut (P3) target lives. **It is a library capability, not a UI toggle** — the web generator always uses faithful-clip (UX decision 2026-06-09: every real tweakcn export is in-gamut, and "faithful vs perceptual" confused devs; §7 spec).
...
- **Colors — all 32.** Emit **every** token in the §5 contract: the 19 core **plus** `chart1…chart5` and the 8 `sidebar*`. `FwColors`'s `const` constructor has no defaults, so a generated `theme.dart` that omits any color **fails to compile** — that is the safety net; never special-case a subset. Map the CSS names directly (`--chart-1` → `chart1`, `--sidebar-foreground` → `sidebarForeground`, …).
- **Radius — emit explicit, additive.** shadcn derives the set **additively** from `--radius` (`sm = r−4px, md = r−2px, lg = r, xl = r+4px`). Emit an **explicit** `FwRadii(base:, sm:, md:, lg:, xl:)` with those values — do **NOT** use `FwRadii.fromBase`, whose `×0.6/0.8/1.0/1.4` factors only coincide with the additive set at the 10px default and diverge for any other `--radius`. (`apps/example/lib/showcase/themes.dart` `_claudeRadii` is the worked reference.)
- **Shadows — 7 named slots.** Map the seven **named** composed shadows by name: `--shadow-2xs`→`xs2`, `--shadow-xs`→`xs`, `--shadow-sm`→`sm`, `--shadow-md`→`md`, `--shadow-lg`→`lg`, `--shadow-xl`→`xl`, `--shadow-2xl`→`xl2`. The **unprefixed `--shadow`** is Tailwind's DEFAULT level (between `sm` and `md`) and is **NOT** `--shadow-md` — tweakcn computes it with the `sm` second-layer formula. `FwShadows` has no DEFAULT slot, so the unprefixed `--shadow` is **dropped knowingly** (corrected 2026-06-09 — the earlier "`--shadow` maps onto `md`" was a wrong-value bug; a future engine enhancement could add a DEFAULT slot). The per-axis `--shadow-x/y/blur/spread/opacity/color` are tweakcn's builder *inputs* that bake into the slot values, not separate outputs; the composed strings are authoritative (this is what makes colored/odd shadows convert for free).
- **Fonts:** the generator emits the `fontFamily` *name* into `FwTypographyTheme(sans:, serif:, mono:, tracking:)` and optional `google_fonts` wiring. It MUST NOT pretend to bundle an arbitrary font. If the family is unknown, emit a clearly-commented `// TODO: bundle this font or map it` rather than a silent fallback.
- **Known generator-phase additions (record, don't silently drop):** a real tweakcn `:root` also carries `--tracking-normal` (theme base letter-spacing) and `--spacing` (base unit). **`--tracking-normal`:** `FwTypographyTheme.tracking` (em, `lerp`-able) **exists as of generator module G0**; the generator emits it. (Stored as an em multiple; converted to `letterSpacing` at the text-apply site.) **`--spacing`:** **descoped** — flutterwindcss's spacing scale is a fixed `1 unit = 4px` (`fwSpace`, matching Tailwind's default), kept context-free by design; the generator drops a non-default `--spacing` *knowingly* and emits a comment noting it. Both are documented here so the generator never silently loses a token (§12).
- **`theme.json`** is the **internal** source of truth (the Dart is a pure function of it, and it drives the preview); its schema mirrors the `FwTokens` shape (32 colors + `radiusBase` + radii + 7 shadows + 3 families + tracking) and is fixed during the generator build. It is computed but not surfaced as a download (see Output above).
...
- The emitted `theme.dart` defines two `const FwTokens` (light/dark) against the §5 contract — see `apps/example/lib/showcase/themes.dart` for a hand-authored example of exactly this output shape. Regenerating MUST be a drop-in file replacement requiring no edits to component code.
