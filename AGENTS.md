...
> **This is the *target* layout.** Today `packages/flutterwindcss/`, `tooling/bake_palette.dart`, `apps/example/`, and `apps/docs/` exist — `apps/example` is a runnable, pure-path **engine** showcase (`WidgetsApp` + `FwAnimatedTheme`), not yet the **component** golden target (no `flutterbits` components yet); `apps/docs` is a **Fumadocs (Next.js/TS) scaffold**, with the docs content + the tweakcn→`theme.dart` generator still to be built. `flutterbits_cli/`, `registry/`, and the melos config are still planned. CI is scoped to `packages/flutterwindcss` only, so the `apps/` projects are not yet built/linted in CI. Don't go looking for dirs that aren't there yet.
