...
- **Shadows — 7 slots + the default.** Emit `FwShadows{xs2,xs,sm,md,lg,xl,xl2}` from `--shadow-2xs…--shadow-2xl`. The **unprefixed `--shadow`** (and `shadow-DEFAULT`) maps onto `md`. The per-axis `--shadow-x/y/blur/spread/opacity/color` are tweakcn's builder *inputs* that bake into the slot values, not separate outputs.
- **Fonts:** the generator emits the `fontFamily` *name* into `FwTypographyTheme(sans:, serif:, mono:)` and optional `google_fonts` wiring. It MUST NOT pretend to bundle an arbitrary font. If the family is unknown, emit a clearly-commented `// TODO: bundle this font or map it` rather than a silent fallback.
...
- **`theme.json`** is the source of truth; its schema mirrors the `FwTokens` shape (32 colors + `radiusBase` + radii + 7 shadows + 3 families [+ tracking once added]) and is fixed during the generator build.
