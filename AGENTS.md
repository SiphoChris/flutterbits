...
- **Known generator-phase additions (record, don't silently drop):** a real tweakcn `:root` also carries `--tracking-normal` (theme base letter-spacing) and `--spacing` (base unit). **`--tracking-normal`:** add a `tracking` field to `FwTypographyTheme` (numeric → `lerp`-able) when the generator lands, and emit it; until then it is a *recorded* gap, not silently dropped. **`--spacing`:** **descoped** — flutterwindcss's spacing scale is a fixed `1 unit = 4px` (`fwSpace`, matching Tailwind's default), kept context-free by design; the generator drops a non-default `--spacing` *knowingly* and emits a comment noting it. Both are documented here so the generator never silently loses a token (§12).
